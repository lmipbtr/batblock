...
with st.container(border=True):
    ...
    with st.form(key='Input_form', clear_on_submit = False):
        ...
        with col1:
            batt_size = st.number_input(label='Enter battery capacity', value = 100)
        batt_chem = st.pills(label='Enter battery chemistry', options = ['LiFePO4','AGM', 'SLA'], default = 'LiFePO4')
        batt_block_size = st.pills(label='Enter Batblock size', options = [25, 50], default = 50)
        solar_panel_rated_current = st.number_input(label='Enter Solar Panel Rated Current', value = 20)
        with col2:
            load_unit = st.pills(label='Enter load unit', options = ['A', 'W'], default = 'A')
        with col1:
            load = st.number_input(label='Enter load', value = 5)
...
if batt_chem == 'LiFePO4':
    recc_batt_dod = 0.8
    part_discharge_time = full_disch_time * 0.8
elif batt_chem == 'AGM':
    recc_batt_dod = 0.5
    part_discharge_time = full_disch_time * 0.5
elif batt_chem == 'SLA':
    recc_batt_dod = 0.5
    part_discharge_time = full_disch_time * 0.5
...
with st.container(border=True):
    ...
    with col3:
        st.text(" hours (to recommended max " + str("{:,.0f}%".format((recc_batt_dod) * 100)) + " DoD)")
    ...
    with col1:
        st.text("Charge Time from " + str("{:,.0f}%".format((1 - recc_batt_dod) * 100)) + " DoD to full:")
    with col2:
    ...
    with col1:
        st.text("Charge Time from empty to full:")
    with col2:
        st.text(str("{0:,.1f}".format(batt_size / batt_block_size)))
    with col3:
        st.text("hours of driving")
    with col1:
        st.text("Charge Time from " + str("{:,.0f}%".format((1 - recc_batt_dod) * 100)) + " DoD to full:")
    with col2:
        st.text(str("{0:,.1f}".format(batt_size * (recc_batt_dod/ batt_block_size))))
    with col3:
        st.text(" hours of driving")
